...
import os
if os.path.exists('../result/port.log'):
    os.remove('../result/port.log')
...
class PortScanner:
    ...
    def __scan_ports(self, ip, message):
        ...
        # root_nood = xml.createElement('root')
        # xml.appendChild(root_nood)
        # ip_nood = xml.createElement('info')
        # root_nood.appendChild(ip_nood)
        output = dict()
        for port in self.targets:
            output[port] = 'CLOSE'
        
        futures = deque()
        f = open('../result/port.log','a')
        ...
        if self.__verbose:
            for port in self.targets:
                if output[port] == 'OPEN':
                    ...
                    f.write('{:10}: {:>10}\n'.format(port_proto, output[port]))
        f.close()
        #             ip_nood.setAttribute('IP',ip)
        #             child = xml.createElement('Port')
        #             child.setAttribute('port',port_proto)
        #             child.setAttribute('status',output[port])
        #             root_nood.appendChild(child)
        # f = open('./result/port.xml','a')
